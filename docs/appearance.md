...
The signature appearance is controlled entirely by JSON option keys passed to
`Atick.signPfx(byte[] pdf, byte[] pfx, String optionsJson)`. By default ATick shows its logo on the
...
new line and `*word*` makes that run **bold**. Because the value lives in a JSON string in Java
source, escape each line break as `\\n`:
...
```{note}
In Java source `\\n` produces the two characters `\n` in the JSON string, which ATick reads as a
line break. A literal Java newline would break the JSON.
```
